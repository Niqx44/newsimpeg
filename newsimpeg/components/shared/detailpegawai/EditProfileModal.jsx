...
export default function EditProfileModal({ open, onClose, onSave, data }) {
  ...
  const handleSave = () => {
    onSave(form);   // ⬅️ update data di parent
    onClose();      // ⬅️ tutup modal
  };

  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
      <div className="bg-[#2b2b2b] p-6 rounded-xl w-full max-w-2xl shadow-lg">
...
function Field({ label, name, value, onChange, type = "text" }) {
  return (
    <div className="flex flex-col gap-1">
      <label className="text-gray-300 text-xs uppercase">
        {label}
      </label>
      <input
        type={type}
